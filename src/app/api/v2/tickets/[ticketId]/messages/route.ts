...
import { addCustomerTicketMessage } from "@/modules/tickets/application/ticket-service";
import { parseIdempotencyKey } from "@/modules/tickets/contracts/idempotency-key";
import { addTicketMessageSchema } from "@/modules/tickets/contracts/ticket-schemas";
...
export async function POST(
  ...
  try {
    ...
    const body = await parseApiV2Json(request, addTicketMessageSchema);
    if (!body.success) return body.response;
    const result = await addCustomerTicketMessage({
      user: auth.user,
      ticketId,
      command: body.data,
    ...
    await recordAuditEvent({
      request,
      action: "TICKET_V2_PUBLIC_MESSAGE_ADD",
      outcome: "SUCCESS",
