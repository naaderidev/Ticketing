...
import { reportingRangeQuerySchema } from "@/modules/reporting/contracts/reporting-kpi-schemas";
...
export async function GET(request: Request) {
  try {
    ...
    const report = await getAutomatedResolutionReport({
    ...
    return handleApiV2Error(
      request,
      error,
      "خطا در دریافت شاخص حل خودکار",
      "GET /api/v2/reporting/kpis/automated-resolution"
    );
  }
}
