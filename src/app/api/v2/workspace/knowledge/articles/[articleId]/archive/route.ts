...
import { archiveKnowledgeArticle } from "@/modules/knowledge/application/knowledge-service";
import { archiveKnowledgeArticleSchema } from "@/modules/knowledge/contracts/knowledge-schemas";
...
export async function POST(
  ...
  try {
    ...
    const body = await parseApiV2Json(request, archiveKnowledgeArticleSchema);
    if (!body.success) return body.response;
    const result = await archiveKnowledgeArticle({ user: auth.user, articleId: path.data });
    ...
    return handleApiV2Error(
      request,
      error,
      "خطا در بایگانی مقاله دانش",
      "POST /api/v2/workspace/knowledge/articles/:articleId/archive"
    );
  }
}
