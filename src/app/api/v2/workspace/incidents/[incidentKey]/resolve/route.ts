...
import { resolveWorkspaceIncident } from "@/modules/incidents/application/incident-service";
import { resolveIncidentSchema } from "@/modules/incidents/contracts/incident-schemas";
...
export async function POST(request: Request, { params }: { params: Promise<{ incidentKey: string }> }) {
  try {
    ...
    const body = await parseApiV2Json(request, resolveIncidentSchema);
    if (!body.success) return body.response;
    const { incidentKey } = await params;
    return apiV2Success(request, await resolveWorkspaceIncident(auth.user, incidentKey, body.data.resolutionSummary));
  } catch (error) {
    if (isIncidentError(error)) return apiV2Error(request, error.message, error.status, error.code);
    return handleApiV2Error(request, error, "خطا در رفع رخداد", "POST /api/v2/workspace/incidents/:incidentKey/resolve");
  }
}
