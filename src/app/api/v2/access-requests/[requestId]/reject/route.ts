...
import { rejectOrganizationAccessRequest } from "@/modules/organizations/application/organization-service";
...
export async function POST(
  ...
  try {
    ...
    const result = await rejectOrganizationAccessRequest({
    ...
    await recordAuditEvent({
      request,
      action: "ORGANIZATION_ACCESS_REQUEST_REJECT",
      outcome: "SUCCESS",
      actorUserId: auth.user.id,
